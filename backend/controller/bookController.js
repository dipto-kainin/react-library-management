...
const addBook = expressAsyncHandler(async (req, res) => {
    ...
    const { title, author, genre, isbnPre, copies, image, description } =
        req.body;
    ...
    if (copies <= 0) {
        res.status(400).json({
            message: "Number of copies should be greater than zero",
        });
        return;
    }
...
const fetchBook = expressAsyncHandler(async (req, res) => {
    try {
        ...
        const books = await Book.find(keywords).select(
            "-isbn -borrowReq -returnReq"
        );
        if (!books) {
            return res.status(404).json({ error: "Books not found" });
        }
...
const adminFetchBook = expressAsyncHandler(async (req, res) => {
    if (req.user.role !== "Admin")
        return res
            .status(401)
            .json({ error: "you are not authorize to access this page" });
    try {
        const { search } = req.params;
        const keywords = {
            $or: [
...
const deleteBook = expressAsyncHandler(async (req, res) => {
    ...
    try {
        ...
        return res
            .json({ message: `${name} book has been deleted` })
            .status(200);
    } catch (err) {
...
const deleteSpecificCopy = expressAsyncHandler(async (req, res) => {
    ...
    try {
        ...
        const result = await Book.updateOne(
            { isbnPre: isbnPre },
            { $pull: { isbn: { $and: [{ id: id }, { borrowedBy: null }] } } }
        );
        if (!result) {
            return res
                .status(400)
                .json({ message: "Book not found or currently borrowed by" });
        }
    ...
    if (req.user.role === "Admin") {
        ...
        if (isAnyCopyBorrowed) {
            return res
                ...
                .json({
                    message:
                        "Cannot update book details while a copy is borrowed",
                });
        }
        ...
        if (req.user._id) {
            ...
            if (
                ...
                book.isbn.some(
                    (copy) =>
                        copy.borrowedBy &&
                        copy.borrowedBy.toString() == req.user._id.toString()
                )
            ) {
                return res
                    .status(200)
                    .json({ message: "You have already requested for borrow" });
            }
...
const borrowReqList = expressAsyncHandler(async (req, res) => {
    if (req.user.role === "Admin") {
        ...
        if (!books.length) {
            return res
                .status(200)
                .json({ message: "No books with borrow requests found" });
        }
...
const borrowReqAccept = expressAsyncHandler(async (req, res) => {
    if (req.user.role === "Admin") {
        ...
        if (!availableCopy) {
            return res
                .status(404)
                .json({ message: "No available copies found" });
        }
        ...
        if (borrowRequestIndex === -1) {
            return res
                .status(404)
                .json({ message: "Borrow request not found" });
        }
...
const returnReq = expressAsyncHandler(async (req, res) => {
    ...
    try {
        ...
        if (
            ...
            return res
                .status(200)
                .json({ message: "You have already requested for return" });
        }
        if (
            !book.isbn.find(
                (copy) => copy.borrowedBy?.toString() == req.user._id.toString()
            )
        ) {
            return res
                .status(401)
                .json({ message: "you do not have the book" });
        }
        book.returnReq.push(req.user._id);
        await book.save();
        return res
            .status(200)
            .json({ message: "return request updated successfully" });
    } catch (err) {
...
const returnReqList = expressAsyncHandler(async (req, res) => {
    if (req.user.role === "Admin") {
        ...
        if (!books.length) {
            return res
                .status(200)
                .json({ message: "No books with return requests found" });
        }
